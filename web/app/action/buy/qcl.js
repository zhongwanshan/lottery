(function () {
    var pageVue = new Vue({
        ...
        methods: {
            ...
            // 随机投注
            randomBuyTime: function (time, num, type) {
                var self = this;

                for (var i = 0; i < time; i++) {
                    (function (i) {
                        setTimeout(function () {
                            var redArr = utility.genreateArr(30);
            ...
            validate: function () {
                ...
                if (self.buyInfo.dBetFunction == 2) {
                    // 红球必须选择7个以上
                    if (self.redBetList.length < 8) {
                        app.showPreloader("必须选择8个以上红球");
                        setTimeout(function () {
                            app.hidePreloader();
                        }, 1000);
                        return false;
            ...
            setOrderInfo: function () {
                ...
                utility.setDigitalOrderInfo(self, self.buyInfo.lotteryTypeId, "七乐彩", function (data, i) {
                    return self.resultList[i]["digitalBetType"] + "|" + self.resultList[i]["dBetFunction"] + "|" + (self.resultList[i].redDAreaList.length == 0 ? "" : self.resultList[i].redDAreaList.join(",")) + "|" + utility.duplicate(self.resultList[i].redBetList, self.resultList[i].redDAreaList).join(",") + "||";
                }, function () {
